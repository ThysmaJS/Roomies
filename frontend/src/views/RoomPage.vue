  ...
  <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 h-screen">
    ...
    <div class="col-span-3 bg-white p-6 overflow-auto">
      <div v-if="room">
        ...
        <!-- Bloc conditionnel pour le jeu -->
        <div class="mt-4">
          <div v-if="room.currentGame === 'morpion'">
            <GameContainer :roomId="roomId" />
          </div>
          <div v-else class="text-center text-gray-500 mt-4">
            <p class="text-lg">🕐 En attente que tous les joueurs soient prêts pour lancer la partie...</p>
          </div>
        </div>
        ...
        <div class="mt-6">
          ...
          <ul class="space-y-2 text-sm text-blue-900">
            <li
              ...
              :key="ru.id"
              class="flex items-center justify-between gap-4 p-2 bg-gray-50 border rounded"
            >
              <div>
              ...
              <!-- ✅ Affichage du bouton "Prêt" si c'est l'utilisateur actuel (par username) -->
              <ReadyToggle
                v-if="ru.user?.username === currentUsername"
                :roomUserId="ru.id"
